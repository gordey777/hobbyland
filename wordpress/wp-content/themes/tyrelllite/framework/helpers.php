...
function theme_after_styles() {
    ...
    <!--[if IE 8]>
        <?php echo "<link href='" . ( get_template_directory_uri() . '/assets/css/iefix.css' ) . "' rel='stylesheet' type='text/css' >"; ?>
    <![endif]--> 
...
function get_logo() { 
    ...
    if (!empty($_SERVER['HTTPS'])) {
        $media_data['logo'] = str_replace('http:', 'https:', $media_data['logo']);
    }
    ...
    <?php else: ?>
        <a id="logo" href="<?php echo esc_url(home_url("/")); ?>"><img alt="Site logo" src="http://dessky.com/tyrell/wp-content/uploads/2013/08/tyrell_logo.png"></a>        
    <?php endif;
}
...
function theme_styles() {	   
    ...
    wp_enqueue_style( "tyrell_woo_style", get_template_directory_uri() . '/framework/woocommerce/style.css' );
    ...
    wp_register_style('dessky_photostream', get_stylesheet_directory_uri() . "/framework/photostream/dessky_photostream.css");
    wp_enqueue_style('dessky_photostream');
}
...
function the_breadcrumb() {
    ...
    if( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) && is_woocommerce() ) {
        ...
        $return_val .= '<a href="' . home_url() .'">' . __("Home", "tyrell") . '</a>' . $divider;
        ...
        if (is_home() && !is_front_page()) {
            ...
            if (is_category() || is_single()) {
                ...
                if (get_search_query() != "") {
                    //printf(__('Search Results for: %s', 'tyrell'), '' . get_search_query() . '');
                } else {
...
function tagsAndAuthor() {
    ...
    <?php echo __('posted by', 'tyrell'); ?> <?php echo get_the_author(); ?>

    <?php
    $posttags = get_the_tags();
    if ($posttags) {
        echo " &nbsp;|&nbsp; ";
        echo __('Taged as', 'tyrell') . " - ";
        $first_tag = true;
...
function newgravatar($avatar_defaults) {
    $myavatar = get_template_directory_uri() . '/assets/images/tyrell_default_avatar.jpg';
    $avatar_defaults[$myavatar] = "Dessky default avatar";
...
function get_logo_and_search() { ?>
    ...
    <div class="right-part">
        ...
        <form role="search" method="get" id="searchform-header" action="<?php echo home_url( '/' ); ?>">
            <input type="text" placeholder="<?php _e("Search", "tyrell"); ?>" value="" name="s" id="s-top" />
        ...
        <?php
            ...
            if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
                ...
                if (function_exists('icl_get_languages')) {
                    ...
                    echo '<span>' . __('Language ', 'tyrell') . '</span>';
...
if (!function_exists('dessky_admin_header_style')) :
    /*
     * Styles the header image displayed on the Appearance > Header admin panel.
     * Referenced via add_custom_image_header() in tyrell_setup().
     */
    function dessky_admin_header_style() {
        ?>
        <style type="text/css">
...
/* Filter wp title */
add_filter('wp_title', 'dessky_filter_wp_title', 10, 2);
function dessky_filter_wp_title($title, $separator) {
    ...
    if (is_search()) {
        ...
        $title = sprintf(__('Search results for %s', 'tyrell'), '"' . get_search_query() . '"');
        // Add a page number if we're on page 2 or more:
        if ($paged >= 2)
            $title .= " $separator " . sprintf(__('Page %s', 'tyrell'), $paged);
    ...
    if ($paged >= 2 || $page >= 2)
        $title .= " $separator " . sprintf(__('Page %s', 'tyrell'), max($paged, $page));
...
/* Page menu show home */
add_filter('wp_page_menu_args', 'dessky_page_menu_args');
function dessky_page_menu_args($args) {
    $args['show_home'] = true;
    return $args;
}

/* Sets the post excerpt length to 40 characters. */
add_filter('excerpt_length', 'dessky_excerpt_length');
function dessky_excerpt_length($length) {
    return 40;
}

/* Returns a "Continue Reading" link for excerpts */
function dessky_continue_reading_link() {
    return ' <a href="' . get_permalink() . '">' . __('Continue reading <span class="meta-nav">&rarr;</span>', 'tyrell') . '</a>';
}

/* Replaces "[...]" (appended to automatically generated excerpts) with an ellipsis and tyrell_continue_reading_link(). */
add_filter('excerpt_more', 'dessky_auto_excerpt_more');
function dessky_auto_excerpt_more($more) {
    return ' &hellip;' . dessky_continue_reading_link();
}

/* Adds a pretty "Continue Reading" link to custom post excerpts. */
add_filter('get_the_excerpt', 'dessky_custom_excerpt_more');
function dessky_custom_excerpt_more($output) {
    if (has_excerpt() && !is_attachment()) {
        $output .= dessky_continue_reading_link();
    }
    return $output;
}
...
add_action('widgets_init', 'dessky_remove_recent_comments_style');
function dessky_remove_recent_comments_style() {
    global $wp_widget_factory;
    remove_action('wp_head', array($wp_widget_factory->widgets['WP_Widget_Recent_Comments'], 'recent_comments_style'));
}

/* Prints HTML with meta information for the current post—date/time and author. */
if (!function_exists('tyrell_posted_on')) :    
    function tyrell_posted_on() {
        printf(__('<span class="%1$s">Posted on</span> %2$s <span class="meta-sep">by</span> %3$s', 'tyrell'), 'meta-prep meta-prep-author', sprintf('<a href="%1$s" title="%2$s" rel="bookmark"><span class="entry-date">%3$s</span></a>', get_permalink(), esc_attr(get_the_time()), get_the_date()
                ), sprintf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>', get_author_posts_url(get_the_author_meta('ID')), sprintf(esc_attr__('View all posts by %s', 'tyrell'), get_the_author()), get_the_author()
                )
        );
    }
endif;
...
if (!function_exists('tyrell_posted_in')) :   
    function tyrell_posted_in() {
        // Retrieves tag list of current post, separated by commas.
        $tag_list = get_the_tag_list('', ', ');
        if ($tag_list) {
            $posted_in = __('This entry was posted in %1$s and tagged %2$s. Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'tyrell');
        } elseif (is_object_in_taxonomy(get_post_type(), 'category')) {
            $posted_in = __('This entry was posted in %1$s. Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'tyrell');
        } else {
            $posted_in = __('Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'tyrell');
        }
...
if (!function_exists('dessky_setup')):
    ...
    function dessky_setup() {
        ...
        load_theme_textdomain('tyrell', get_template_directory() . '/lang');
        ...
        register_nav_menus(array(
            'primary' => __('Primary Navigation', 'tyrell'),
        ));
        ...
        define('HEADER_IMAGE', '%s/assets/images/headers/path.jpg');

        // The height and width of your custom header. You can hook into the theme's own filters to change these values.
        // Add a filter to tyrell_header_image_width and tyrell_header_image_height to change these values.
        define('HEADER_IMAGE_WIDTH', apply_filters('tyrell_header_image_width', 190));
        define('HEADER_IMAGE_HEIGHT', apply_filters('tyrell_header_image_height', 54));
        ...
        // custom headers. See tyrell_admin_header_style(), below.
        //add_custom_image_header( '', 'tyrell_admin_header_style' );
        // ... and thus ends the changeable header business.
        // Default custom headers packaged with the theme. %s is a placeholder for the theme template directory URI.
        register_default_headers(array(
            'berries' => array(
                'url' => '%s/assets/images/headers/logo.png',
                'thumbnail_url' => '%s/assets/images/headers/logo.png',
                /* translators: header image description */
                'description' => __('Tyrell default logo', 'tyrell')
            )
        ));
        ...
        if ($_GET['activated'] == 'true' && $theme->get_template() == 'tyrell132') {
            ...
            update_option('dessky_contact', $arr);
        } 
    }
...
function MailFunction(){
    $to = $_POST['form_data']['dessky-admin-mail'];    //your e-mail to which the message will be sent
    $from = $_POST['form_data']['dessky-admin-mail'];        //e-mail address from which the e-mail will be sent
    ...
    foreach ($_POST['form_data'] as $postname => $post) {
        ...
        if ($postname != 'dessky-admin-mail' && $postname != "recaptcha_challenge_field" && $postname != "recaptcha_response_field") {
...
/*
	Tyrell Paid Version Notice
*/

add_action('admin_notices', 'tyrell_paid_admin_notice');

function tyrell_paid_admin_notice() {
	global $current_user ;
        $user_id = $current_user->ID;
        /* Check that the user hasn't already clicked to ignore the message */
	if ( ! get_user_meta($user_id, 'tyrell_paid_ignore_notice') ) {
        echo '<div class="updated"><p>'; 
        printf(__('If You like this theme You\'re gonna love full Tyrell version! <strong>It comes with the Unlimited color options, Shortcodes, Google and Custom fonts, Sidebars, Portfolio and Testimonials Area!</strong>  Best of all, it is <strong>only $29</strong>. <a href="http://dessky.com/themes-list/tyrell-responsive-woocommerce-theme/" target="blank"><strong>Click here to learn more.</strong></a> | <a href="%1$s">Hide Notice</a>'), '?tyrell_paid_nag_ignore=0');
        echo "</p></div>";
	}
}

add_action('admin_init', 'tyrell_paid_nag_ignore');

function tyrell_paid_nag_ignore() {
	global $current_user;
        $user_id = $current_user->ID;
        /* If user clicks to ignore the notice, add that to their user meta */
        if ( isset($_GET['tyrell_paid_nag_ignore']) && '0' == $_GET['tyrell_paid_nag_ignore'] ) {
             add_user_meta($user_id, 'tyrell_paid_ignore_notice', 'true', true);
	}
}