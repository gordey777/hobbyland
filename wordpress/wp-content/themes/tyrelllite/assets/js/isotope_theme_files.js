jQuery(function($){
    ...
    if( $(".one-column").length == 0 ) {
        $(".single-post2").each(function(index) {res_ch(index);});
        $(window).resize(function() {$(".single-post2").each(function(index) {res_ch(index);});});
        $(".post .wp-post-image, .single-post2 img").load(function(){$(".single-post2").each(function(index) {res_ch(index);});});
    }
    ...
    $(".pagination-value.current").click();
});
