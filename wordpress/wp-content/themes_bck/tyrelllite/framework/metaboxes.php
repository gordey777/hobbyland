...
add_action('add_meta_boxes', 'dessky_content_add_custom_box');
add_action('save_post', 'dessky_content_save_postdata');

function dessky_content_add_custom_box() {

    $screens = array('portfolio');
    foreach ($screens as $screen) {
        add_meta_box(
                'dessky_breadcrumbs_sectionid', __('Side Content', 'tyrell'), 'display_meta_box_content', $screen
        );
    }
}
...
function dessky_content_save_postdata($post_id) { 
